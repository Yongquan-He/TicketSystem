...
public class Test {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		final TicketingDS tds = new TicketingDS(routenum, coachnum, seatnum, stationnum);
	    	...
	    	threads[i] = new Thread(new Runnable() {
                public void run() {
            		...
            		for (int i = 0; i < testnum; i++) {
            			...
            			if (0 <= sel && sel < retpc && soldTicket.size() > 0) { // return ticket 0-30
            				int select = rand.nextInt(soldTicket.size());
            				if ((ticket = soldTicket.remove(select)) != null) {
            					if (tds.refundTicket(ticket)) {
            						System.out.println("TicketRefund" + " " + ticket.tid + " " + ticket.passenger + " " +
            					ticket.route + " " + ticket.coach  + " " + ticket.departure + " " + ticket.arrival + " " + ticket.seat);
            					} else {
            			...
            			} else if (retpc <= sel && sel < buypc) { // buy ticket 30-60
            				...
            				if ((ticket = tds.buyTicket(passenger, route, departure, arrival)) != null) {
            					...
            					System.out.println("TicketBought" + " " + ticket.tid + " " + ticket.passenger + " " + ticket.route + " " + 
            					ticket.coach + " " + ticket.departure + " " + ticket.arrival + " " + ticket.seat);
            				} else {
            					System.out.println("TicketSoldOut" + " " + route+ " " + departure+ " " + arrival);
            				}
            			} else if (buypc <= sel && sel < inqpc) { // inquiry ticket  60-100
            				int route = rand.nextInt(routenum) + 1;
            				int departure = rand.nextInt(stationnum - 1) + 1;
            				int arrival = departure + rand.nextInt(stationnum - departure) + 1; // arrival is always greater than departure
